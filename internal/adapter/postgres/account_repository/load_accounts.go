...
func (r repository) LoadAccounts() (map[uint]*domain.AccountInformation, error) {
	...
	rows, err := r.db.Query(
		...
		accounts."limit", 
		(accounts.initial_balance - COALESCE(SUM(CASE WHEN transactions.status = 1 THEN transactions.amount ELSE 0 END), 0)) AS balance
		FROM accounts
		LEFT OUTER JOIN transactions ON (transactions.account_id = accounts.id)
		GROUP BY accounts.id, accounts.initial_balance, accounts."limit"`,
	)
	if err != nil {
